...
public abstract class GenericDaoAbstract<T extends BasicEntity> {
	 ...
	 *
	 * @param t
	 *            - Objeto a set alterado ou inserido.
		...
		try {
			if (t.getId() == null) {
				...
				if (!session.contains(t)) {
					if (session.get(t.getClass(), t.getId()) == null) {
						throw new Exception("Erro ao atualizar objeto da classe " + t.getClass());
					}
				}
	 ...
	 *
	 * @param clazz
	 *            - Classe do Objeto a ser consutlado
	 * @param id
	 ...
	 * consultada tenha uma consulta nomeada com o nome da classe, + ".searchAll".
	 * Por exemplo uma entidade Usuario: "Usuario.searchAll".
	 *
	 * @param className
	 *            Nome da classe buscada
	 ...
	 *
	 * @param message
	 *            - menasgem a ser enviada por e-mail
	 */
